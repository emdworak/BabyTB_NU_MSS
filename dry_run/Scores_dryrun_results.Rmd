...
```{r, eval = FALSE}
save_cor <- data.frame(rownames(temp_cor), temp_cor)

write_xlsx(save_cor, "output/cor_scores_ef_cog.xlsx")
```
...
write_xlsx(save_cor, "output/cor_scores_language.xlsx")
```
...
write_xlsx(save_cor, "output/cor_scores_numeracy.xlsx")
```
...
write_xlsx(save_cor, "output/cor_scores_social_obs.xlsx")
```
...
write_xlsx(save_cor, "output/cor_scores_mullen.xlsx")
```


### Motor
Miriam wants the distance
...
limited_item_df <- full_join(item_wide, dryRun_Registration_Age, 
          by = "pin",
          multiple = "all") %>% 
  filter(instrument_title == "Get Up and Go") %>% 
  select(-c(registration_id, pid, assessment_name, age)) %>% 
  full_join(., dryRun_dp4, by = "pin") %>% 
  select(-c(child_id, age, gender, age_at_testing, administration_form_id)) %>% 
  distinct() %>% 
  filter(pin %in% analysis_ids) 
...
temp_cor <- temp_cor[-c(3:6), c(1, 2)]

temp_cor <- cbind(n = temp_count[-c(1:6), 1], temp_cor)

temp_cor  %>% 
kbl(caption = "Correlation of Motor with Age and DP4") %>% 
  kable_styling() 
```
...
write_xlsx(save_cor, "output/cor_scores_motor.xlsx")
```
