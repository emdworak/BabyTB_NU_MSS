...
files <- list.files(folder, pattern = "^ItemExportNarrowStructure")

length(files) # There should be 175 cases

combine_ItemExportNarrow <- list()
...
for(i in 1:length(files)){
  ...
  combine_ItemExportNarrow[[i]] <- item_info
}

dryRun_ItemExportNarrow <- bind_rows(combine_ItemExportNarrow)
...
full_join(pin_check, dryRun_ItemExportNarrow, by = c("RegistrationID"),
...
temp2 <- full_join(pin_check, dryRun_ItemExportNarrow, by = c("RegistrationID"),
                   suffix = c("_check", ""), multiple = "all") %>% 
  select(PIN) %>% 
  unique() %>% 
  pull(PIN)
...
dryRun_ItemExportNarrow_nodups <- full_join(pin_check, dryRun_ItemExportNarrow, by = c("RegistrationID"),
          suffix = c("_check", ""), multiple = "all") %>% 
...
dryRun_ItemExportNarrow_nodups %>% 
  count(PIN, import_value) %>% 
  arrange(PIN) %>% 
  count(PIN) %>% 
...
final_dryRun_ItemExportNarrow <- dryRun_ItemExportNarrow_nodups %>% 
  filter(PIN %in% analysis_ids) %>% 
  select(-c(import_value, PID_check))

final_dryRun_ItemExportNarrow %>% 
  count(PIN)

write_csv(final_dryRun_ItemExportNarrow, 
          file = "/Volumes/fsmresfiles/MSS/Research/Projects/Baby_Toolbox/Data/Combined_Dry_Run_Data/dryRun_ItemExportNarrow_20230323.csv")
