...
cert_df_app_v5v2 %>% 
  count(name, site) %>% 
  filter(name %in% c("Ramirez, Rosario", "Ramirez,Rosario",
                     "Maggie Craig", "Maggie M Craig",
                     "Fletes, Fabiola", "Fletes. Fabiola",
                     "Yilma,aden", "Yilma.aden",
                     "medina, keila", "Medina, Keila"
                     )) %>% 
...
Fix the names:
```{r}
cert_df_app_v5v2$name[cert_df_app_v5v2$name == "Ramirez,Rosario"] <- "Ramirez, Rosario"

cert_df_app_v5v2$name[cert_df_app_v5v2$name == "Maggie M Craig"] <- "Maggie Craig"

cert_df_app_v5v2$name[cert_df_app_v5v2$name == "Fletes. Fabiola"] <- "Fletes, Fabiola"

cert_df_app_v5v2$name[cert_df_app_v5v2$name == "Yilma.aden"] <- "Yilma, Aden"
...
```{r}
cert_df_app_v5v2$site[cert_df_app_v5v2$name == "Yohe, Cameron"] <- "Houston, TX"

cert_df_app_v5v2$site[cert_df_app_v5v2$name == "stokes,monique"] <- "Phoenix, AZ"
...
```{r, eval = FALSE}
sas_round2 <- scored_cert_df %>%
  select(name, site, starts_with("ScoreOverall")) %>% 
  pivot_longer(cols = ScoreOverall_sas_v5:ScoreOverall_sas_v8,
               names_to = "names") %>% 
...
sas_round2 <- cbind(test = rownames(sas_round2), sas_round2)

write_xlsx(sas_round2, "for_Miriam/sas_round2.xlsx")

```

```{r, eval=FALSE}
sas_cert_summary <- scored_cert_df %>%
  select(name, site, starts_with("ScoreOverall")) %>%
  rename_with(stringr::str_replace, 
              pattern = "ScoreOverall\\_", replacement = "", 
              matches("ScoreOverall\\_")) %>%
  group_by(name, site) %>% 
  mutate(sas_r2_avg = mean(c(sas_v5, sas_v6, sas_v7, sas_v8)),
         sas_r2_pass_fail = ifelse(sas_r2_avg >=.750, "Pass", "Fail")
  ) %>% 
  select(name, site, 
         starts_with("sas_v"), starts_with("sas_r2")
         ) %>% 
  data.frame() 

write_xlsx(sas_cert_summary, "for_Miriam/20230614_sas_cert_summary.xlsx")
```
